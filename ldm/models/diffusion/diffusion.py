...
class GaussianDiffusion(nn.Module):
    ...
    def weight_sample_loop(self, shape, cond, neigh, r, s_cbf=1, s_clf=1, p=1, mode='min', 
        ...
        for i in reversed(range(0, self.n_timesteps)):
            ...
            if mode=='min':
                # dis = dx**2 + dy**2
                # min_idx = torch.argmin(dis, axis=1).squeeze()
                ...
                # weight[:, :2, :]=torch.zeros(1,2,160).cuda()
            elif mode=='mean':
                dWcbf = dWcbf.mean(1)
            elif mode=='sum':
                dWcbf = dWcbf.sum(1)
            ...
            if apply_clf:
                ...
                x = self.p_sample_weight(x, cond, timesteps, dWcbf, s_cbf, 0, 0, apply_norm=apply_norm)
            ...
            if (i%20) == 0 and apply_cbf_mid:
                ...
                if min < -1:
                    ...
                    h = np.zeros((horizon))
                    # dis = dx**2 + dy**2
                    # min_idx = torch.argmin(dis, axis=0)
                    min_idx = torch.argmin(Wcbf, axis=1).squeeze()
                    for j in range(horizon):
        ...
        if apply_cbf:
            ...
            # dis = dx**2 + dy**2
            # min_idx = torch.argmin(dis, axis=0)
            min_idx = torch.argmin(V, axis=1).squeeze()
            for j in range(horizon):
                G[j][2*j] = -2 * dx[min_idx[j], j]
                G[j][2*j+1] = -2 * dy[min_idx[j], j]
                h[j] = (dx[min_idx[j], j]**2 + dy[min_idx[j], j]**2 - r**2)**p
    ...
    # @torch.no_grad()
    # def weight_sample_loop2(self, shape, cond, neigh, r, s, p=1, mode='min', 
    #                        apply_norm=False, apply_cbf=False, apply_cbf_mid=False, incl_vel=True, verbose=True, return_diffusion=False):
    #     # x: [bs, 4, time]
    #     # neigh: [bs, n_neigh, 2, time]
    ...
    #     x[:,:2,0]=cond[:,:,0].clone()
    #     x[:,:2,-1]=cond[:,:,1].clone()
    #     # x[:,2:,0]=torch.zeros(2).cuda()
    #     # x[:,2:,-1]=torch.zeros(2).cuda()
    #     n_neigh = neigh.shape[1]
    #     horizon = x.shape[-1]
    ...
    #         V[V.isnan()]=torch.inf
    #         dV = torch.zeros(1,n_neigh,4,horizon).cuda()

    #         dV[:,:,0,:] = (-b**(3/2))*dx*c + (b**(-1/2))*dvx + p*(b**(p/2-1))*dx
    #         dV[:,:,1,:] = (-b**(3/2))*dy*c + (b**(-1/2))*dvy + p*(b**(p/2-1))*dy
    ...
    #             # min_idx = torch.argmin(dis, axis=1).squeeze()
    #             min_idx = torch.argmin(V, axis=1).squeeze()
    #             weight = dV[:, min_idx, :, torch.arange(dV.size(-1))]
    ...
    #         x = self.p_sample_weight(x, cond, timesteps, weight, s, apply_norm=apply_norm)
    ...
    #                 for j in range(horizon):
    #                     G[j][2*j] = -((b[:,:,j]+0.1)**(-1/2)) * dx[min_idx[j], j]
    #                     G[j][2*j+1] = -((b[:,:,j]+0.1)**(-1/2)) * dy[min_idx[j], j]
    #                     h[j] = (dx[min_idx[j], j]**2 + dy[min_idx[j], j]**2 - r**2)**(p/2)
    #                 G = matrix(G)
    #                 h = matrix(h)
    #                 sol = cvxopt.solvers.qp(P=P,q=f,G=G,h=h)
    #                 x[:,2:,:]=torch.tensor(np.array(sol["x"])).reshape((1,horizon,2)).transpose(1,2)
    ...
    @torch.no_grad()
    def energy_sample_loop(self, shape, cond, neigh, r, s, p=1, mode='min', apply_cbf=False, verbose=True, return_diffusion=False):
        # x: [bs, 4, time]
        # neigh: [bs, n_neigh, 2, time]
        ...
        for i in reversed(range(0, self.n_timesteps)):
            ...
            dy = x[:,1,:] - neigh[:,:,1,:]
            b = dx**2 + dy**2 - r**2
            V = 2*dx*x[:,2,:] + 2*dy*x[:,3,:] + b**p
            V[V.isnan()]=torch.inf
            V[V>=0]=0
            E = V**2 # shape: [batch, n_neigh, time]

            if mode=='min':
                dis = dx**2 + dy**2
                # min_idx = torch.argmin(dis, axis=1).squeeze()
                E, _ = torch.max(E, axis=1)
            elif mode=='mean':
                E = E.mean(1)
            elif mode=='sum':
                E = E.sum(1)
    ...
    def weight_sample(self, cond, neigh, radius, s_cbf=1, s_clf=-1, p=1, mode='min', 
        ...
        return self.weight_sample_loop2(shape, cond, neigh, radius, s_cbf, s_clf, p, mode, 
