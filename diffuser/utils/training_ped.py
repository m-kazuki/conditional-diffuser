...
class Trainer(object):
    ...
    def train(self, epoch):
        ...
        for traj in self.dataloader:
            ...
            cond = traj[:,:2,(0,-1)]
            loss = self.model.loss(traj, cond) # loss.shape: scalar, infos[dict]: {a0_loss: 0.5789} <- action loss
            loss.backward()
            ...
            if self.step % self.log_freq == 0:
                ...
                with open(logpath, 'a') as f:
                    log = f'(Epoch {epoch}) {self.step} steps: {loss:8.4f}\n'
                    f.write(log)
