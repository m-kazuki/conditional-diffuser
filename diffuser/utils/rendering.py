...
class MazeRenderer:
    ...
    def renders(self, observations, conditions=None, title=None, obst_pos=None, obst_rad=None):
        plt.clf()
        fig = plt.gcf()
        ...
        if obst_pos is not None:
            ...
            x = obst_rad[0] * np.cos(theta) + obst_pos[1]
            y = obst_rad[1] * np.sin(theta) + obst_pos[0]
            plt.plot(x, y, c='r')
        plt.axis('off')
        plt.title(title)
    ...
    def composite(self, savepath, paths, ncol=5, obst_pos=None, obst_rad=None, **kwargs):
        '''
            savepath : str
        ...
        for path, kw in zipkw(paths, **kwargs):
            img = self.renders(*path, **kw, obst_pos=obst_pos, obst_rad=obst_rad)
            images.append(img)
...
class Maze2dRenderer(MazeRenderer):
    ...
    def renders(self, observations, conditions=None, obst_pos=None, obst_rad=None, **kwargs):
        ...
        if len(bounds) == 2:
            ...
            if obst_pos is not None:
                ...
                obst_pos[0] /= iscale
                obst_pos[1] /= jscale
                rad_x = obst_rad/jscale
                rad_y = obst_rad/iscale
                obst_rad = [rad_x, rad_y]
        else:
            raise RuntimeError(f'Unrecognized bounds for {self.env_name}: {bounds}')

        if conditions is not None:
            conditions /= scale
        return super().renders(observations, conditions, obst_pos=obst_pos, obst_rad=obst_rad, **kwargs)
