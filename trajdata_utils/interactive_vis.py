...
def plot_agent_batch_interactive(batch: AgentBatch, batch_idx: int, cache_path: Path):
    ...
    palette = sns.color_palette("hls", 24).as_hex()
    ...
    for n_neigh in range(num_neighbors):
        if neigh_types[n_neigh] == AgentType.VEHICLE:
            neigh_colors.append(palette[10])
        elif neigh_types[n_neigh] == AgentType.PEDESTRIAN:
            neigh_colors.append(palette[11])
        elif neigh_types[n_neigh] == AgentType.BICYCLE:
            neigh_colors.append(palette[12])
        else:
            neigh_colors.append(palette[13])
